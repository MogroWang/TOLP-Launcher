...
《光点之旅（Tour of Light Point）》的桌面启动器：启动本地目录下的网页版游戏，并提供游戏版本管理、启动方式（全屏 / 窗口化 + 可调窗口分辨率）等设置。
...
- 点击「启动游戏」会按启动设置**新开一个独立游戏窗口**：全屏（无边框）或窗口化（分辨率可在启动器设置中选择预设或自定义）；关闭游戏窗口即退出游戏，启动器不受影响。
- 主界面实时显示游戏运行状态：游戏窗口启动后主按钮变为红色「取消运行」（点击关闭游戏），状态行小绿点呼吸闪烁；游戏关闭后自动恢复「启动游戏」。
- 游戏存档（localStorage 等浏览器数据）写入数据文件夹的 `saves/`，随启动器目录移动。
...
启动器首次运行会在 exe 同目录创建**数据文件夹 `launcher-data/`**（游戏版本与存档的统一存放位置）。快速启动页可用下拉列表选择启动版本：**4.0.002 DEV** 或 **自定义启动**。

- **内置版本 4.0.002 DEV**：按以下顺序解析游戏目录：
  1. 数据文件夹 `launcher-data/versions/4.0.002/`（版本管理页会自动识别 `versions/` 下所有含 `index.html` 的子目录）；
  2. 版本管理中指定的**自定义位置**；
  3. 旧版约定：exe 同目录 `games/<版本>/`；
  4. exe 同目录 `game/` 文件夹（便携版默认约定）；
- **自定义启动**：按以下顺序查找游戏目录：

1. 「自定义启动 → 选择文件夹」中指定的目录；
2. 启动器 exe 同目录的 `game/` 文件夹（便携版默认约定）；
3. 开发构建（`tauri dev`）下回退到仓库根目录的 `game/`。

要求目录中直接包含 GDevelop「网页版 / HTML5」导出的 `index.html`。若目录中带有 `manifest.webmanifest`，启动器会读取其中的 `version`（版本号）与 `id`（官方 id 为 `com.mws.tolp`）用于版本识别。也可以用 `dev/sample-game/` 里的测试页验证整条链路。
...
```
├─ src/                  # Vue 前端
│  ├─ components/        # 标题栏 / 侧边导航 / 快速启动 / 版本管理 / 启动器设置 / 关于
│  └─ lib/tauri.ts       # 后端命令封装
├─ src-tauri/            # Tauri + Rust 后端
│  └─ src/
│     ├─ data_dir.rs     # 数据文件夹（versions/ 游戏版本、saves/ 游戏存档）
│     ├─ game_server.rs  # 本地静态服务器
│     ├─ launcher.rs     # 游戏目录解析、版本识别与窗口启动
